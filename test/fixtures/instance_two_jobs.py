...
def create_test_instance(delivery_window_right_margin=0, max_lag=100):
    """
    Set up an instance with 2 vehicle and 4 deliveries, 1 completed 1 partially completed and the last two not completed
    stop_time = 1
    service_time = 3
    stop_cost = 2
    ...
    There are two job present:
    Job1: tasks [0, 2, 1, 3],the arrival times of the competed task are:
        - 0 -> 0
        - 2 -> 9
        - 1 -> 18
        - 3 -> 37
    ...
    _, job0 = instance.create_vehicle(
    ...
    _, job1 = instance.create_vehicle(
        vehicle_type_id=vehicle_type_1.id,
        max_weight=15,
        max_volume=9,
        unit_distance_cost=3,
    ...
    instance.plan.set_tasks(job_id=job0.id, route=[(0, 0, 4), (2, 9, 13), (1, 18, 22), (3, 39, 43)])
